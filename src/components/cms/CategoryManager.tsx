      ...
      {catsQ.isLoading ? (
        ...
        <div ref={listRef} className="space-y-2">
          {cats.map((c, i) => {
            ...
            return (
              <div key={c.id} className="flex items-center gap-3 rounded-xl bg-white p-3 ring-1 ring-black/5">
                ...
                {editingThis ? (
                  <div className="flex flex-1 flex-wrap items-center gap-2">
                    ...
                    <button onClick={() => saveEdit(c.id)} className="text-green-600" aria-label="Save"><Check className="h-5 w-5" /></button>
                  ...
                  <>
                    ...
                    <div className="flex items-center gap-1 text-neutral-400">
                      <button onClick={() => move(i, -1)} disabled={i === 0} className="disabled:opacity-30" aria-label="Move up"><ChevronUp className="h-4 w-4" /></button>
                      <button onClick={() => move(i, 1)} disabled={i === cats.length - 1} className="disabled:opacity-30" aria-label="Move down"><ChevronDown className="h-4 w-4" /></button>
                      <button onClick={() => { setEditId(c.id); setEditName(c.name); setEditIcon(c.icon ?? CATEGORY_ICON_NAMES[0]) }} className="ml-1 hover:text-neutral-700" aria-label="Edit"><Pencil className="h-4 w-4" /></button>
                      <button onClick={() => remove(c)} className="hover:text-red-500" aria-label="Delete"><Trash2 className="h-4 w-4" /></button>
                    </div>
                  </>
                )}
              </div>
